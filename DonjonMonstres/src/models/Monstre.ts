...
const AventurierSchema = new Schema({
    ...
    rang : {type : String, required : [true, "Le rang de l'aventurier est obligatoire (paysan, bourgeois, noble, royal)"], enum : [rangAventurier, "Les valeurs acceptées sont 'paysan', 'bourgeois', 'noble', 'royal'"]},
    classe : {type : String, required : [true, "La classe de l'aventurier est obligatoire (guérrier, mage, voleur, barde)"], enum : [classeAventurier, "Les classes acceptés sont 'guérrier', 'mage', 'voleur', 'barde'"]}
})
...
const MonstreSchema = new Schema<IMonstre>({
    ...
    niveau : {type : Number, required : [true, "Le niveau du monstre est nécéssaire"], validate : [ (niveau : Number) => { return niveau.valueOf() > 0; }, "Le niveau ne peut pas être égal ou inférieur à 0"] },
